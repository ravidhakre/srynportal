...
export const metadata: Metadata = {
  title: "Career & Recruitment Blog | SRYN Recruitment",
  description: "Career advice, interview tips, hiring guides, and workplace insights.",
};

async function getRecruitmentBlogs(): Promise<BlogDocument[]> {
  try {
    const db = getFirebaseDb();
    const q = query(
      collection(db, COLLECTIONS.BLOGS),
      where("businessVertical", "==", "RECRUITMENT"),
    ...
    console.warn("Could not fetch recruitment blogs:", err);
    return [];
  }
}

export default async function BlogListPage() {
  const blogs = await getRecruitmentBlogs();
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12 text-left">
      <section className="text-center space-y-4 max-w-3xl mx-auto">
        ...
        <h1 className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight">Career & Hiring Insights</h1>
        <p className="text-slate-300 text-lg leading-relaxed">
          Guides for job seekers on interview preparation, resume formatting, and employer insights on talent retention.
        </p>
      </section>
      ...
      {blogs.length === 0 ? (
        <Card className="bg-slate-900 border-slate-800 p-12 text-center max-w-xl mx-auto space-y-4 shadow-2xl">
          ...
          <p className="text-slate-400 text-sm">
            Our recruitment editorial team is preparing career guides and hiring insights. Check back soon!
          </p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {blogs.map((post) => (
            <Card key={post.id} className="bg-slate-900 border-slate-800 p-6 space-y-4 flex flex-col justify-between">
              ...
              <Link href={`/blog/${post.slug}`} className="text-xs font-semibold text-emerald-400 hover:underline">
                Read Article →
              </Link>
            </Card>
          ))}
