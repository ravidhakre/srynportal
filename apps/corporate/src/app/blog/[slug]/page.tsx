...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${slug.replace(/-/g, " ")} | SRYN Insights`,
    description: "Read article insights from SRYN Management Pvt. Ltd.",
  };
}
...
async function getBlogPost(slug: string): Promise<BlogDocument | null> {
  try {
    ...
    const q = query(collection(db, COLLECTIONS.BLOGS), where("slug", "==", slug), where("isPublished", "==", true));
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8 text-left">
      ...
      <div className="space-y-3">
        <Badge variant="tech">{post.category || "Insight"}</Badge>
        <h1 className="text-3xl sm:text-4xl font-extrabold text-white">{post.title}</h1>
        <p className="text-xs text-slate-500">By {post.authorName || "SRYN Editorial Team"}</p>
      </div>
