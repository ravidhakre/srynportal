...
export const metadata: Metadata = {
  title: "Technology Blog & Engineering Articles | SRYN Technology",
  description: "Read articles and insights on web development, software engineering, cloud architecture, and digital marketing from SRYN Technology.",
};

async function getTechnologyBlogs(): Promise<BlogDocument[]> {
  try {
...
export default async function BlogListPage() {
  const blogs = await getTechnologyBlogs();

  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12 text-left">
      <section className="text-center space-y-4 max-w-3xl mx-auto">
        <Badge variant="tech">ENGINEERING ARTICLES</Badge>
        <h1 className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight">Technology Blog</h1>
        <p className="text-slate-300 text-lg leading-relaxed">
          Technical guides, software architecture insights, and digital growth strategies.
        </p>
      </section>
      ...
      {blogs.length === 0 ? (
        <Card className="bg-slate-900 border-slate-800 p-12 text-center max-w-xl mx-auto space-y-4 shadow-2xl">
          ...
          <p className="text-slate-400 text-sm">
            Our engineering team is preparing articles on web performance, Next.js, and business automation. Check back soon!
          </p>
        </Card>
      ) : (
